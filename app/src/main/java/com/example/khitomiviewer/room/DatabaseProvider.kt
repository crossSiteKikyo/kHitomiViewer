...
object DatabaseProvider {
    ...
    val MIGRATION_4_5 = object : Migration(4, 5) {
        override fun migrate(database: SupportSQLiteDatabase) {
            ...
            database.execSQL("ALTER TABLE `gallery` ADD COLUMN `lastReadAt` INTEGER NOT NULL DEFAULT 0")
            // 2. lastReadPage 컬럼 추가 (기본값 0)
            database.execSQL("ALTER TABLE `gallery` ADD COLUMN `lastReadPage` INTEGER NOT NULL DEFAULT 0")
            // 3. 기록 정렬용 인덱스 추가
            database.execSQL("CREATE INDEX IF NOT EXISTS `index_gallery_lastReadAt` ON `gallery` (`lastReadAt`)")
        }
    }
